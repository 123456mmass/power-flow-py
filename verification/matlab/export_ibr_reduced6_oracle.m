function payload=export_ibr_reduced6_oracle(output_file,source_root)
%EXPORT_IBR_REDUCED6_ORACLE Export GFL/GFM reduced-six SMIB evidence.
if nargin<1 || isempty(output_file),error('export_ibr_reduced6_oracle:output','Output required.');end
...
ids={'gfl_reduced6_smib','gfm_reduced6_smib'};
items=repmat(struct('case_id','','kind','','state_names',{{}},'x0',[],'y0',[], ...
    'u0',[],'Vinf_real',0,'Vinf_imag',0,'Z_real',0,'Z_imag',0, ...
    'f0',[],'g0',[],'fx',[],'fy',[],'gx',[],'gy',[],'A',[], ...
    'eigen_real',[],'eigen_imag',[],'t',[],'x_drift',[],'y_drift',[], ...
    'x_perturbed',[],'y_perturbed',[],'newton_residual',[],'max_drift',0),2,1);
for k=1:2
    ...
    items(k)=struct('case_id',ids{k},'kind',c.smib_verification.kind, ...
        ...
        'Z_real',real(s.Z_line),'Z_imag',imag(s.Z_line),'f0',s.f0(:).', ...
        'g0',s.g0(:).','fx',s.fx,'fy',s.fy,'gx',s.gx,'gy',s.gy,'A',s.A, ...
        'eigen_real',real(s.eigenvalues(:)).','eigen_imag',imag(s.eigenvalues(:)).', ...
        't',t.tgrid(:).','x_drift',t.x_drift.','y_drift',t.y_drift.', ...
...
payload=struct('schema','power-flow-py/matlab-ibr-reduced6-oracle/1.0', ...
...
fid=fopen(output_file,'w');if fid<0,error('export_ibr_reduced6_oracle:open','Open failed.');end
file_cleanup=onCleanup(@()fclose(fid)); %#ok<NASGU>
fprintf(fid,'%s',jsonencode(payload,'PrettyPrint',true));
fprintf('Wrote reduced-six IBR oracle: %s\n',output_file);
end
